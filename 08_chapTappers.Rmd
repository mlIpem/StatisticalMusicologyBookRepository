...
```{r init_chapFingTapEntrainment1, warning=FALSE, message = FALSE}
...
```{r chapTappers1framework, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Methodological approach (see text for explanation)', message=FALSE, warning=FALSE, out.width='90%'}

knitr::include_graphics("Figures/chapTappers_AnalysisPlan.jpg")
...
```{r chapTappers2ODEsystem, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Dynamic system', message=FALSE, warning=FALSE, out.width='75%'}

knitr::include_graphics("Figures/chapTappers_odepredictor.jpg")
# \@ref(fig:chapTappers2ODEsystem)
```
...
```{r chapTappers2ODEsystem2, echo=FALSE, fig.align='center', fig.asp=.75, fig.cap='Assumed coupling strength parameters of the uncoupled and coupled condition', message=FALSE, warning=FALSE, out.width='100%'}

knitr::include_graphics(
  "Figures/chapTappers_odesystem2parts.jpg")
# \@ref(fig:chapTappers2ODEsystem2)
```
...
```{r chapTappersSol1, echo=FALSE, fig.align='center', fig.cap='Error terms in s1 due to competing influences of m1 and s2. The error terms are shown as thin lines. The columns show five different values of ks1m1 (1, .8, .6, .4, .2), the rows show two different values of ks2m2 (1, .75). The thick line is the average of the thin lines. Overall, these figures show the instantaneous frequency (without the eigen-frequency) of the error terms contribution to s1', message=FALSE, warning=FALSE, out.width='100%', fig.show = "hold"}
...
for (ks1m1 in seq(1,.2,-.2)){
  for(ks2m2 in c(1,.75)){
    parm <- c(ks1m1 = ks1m1,ks2m2 = ks2m2, ds1m1 = 0, ds2m2 = 0, ds1s2 = 0, ds2s1 = 0)
...
Solution %>% group_by(time) %>% ggplot() +
  geom_line(aes(x= time,y = dem1s1), color = "lightblue") +
  geom_line(aes(x= time,y = des2s1), color = "lightgreen") +
  geom_line(aes(x= time,y = .5*(dem1s1 + des2s1)), color = "#56B4E9",  size=1) +
...
```{r chapTappersPhase2, fig.align='center', fig.cap='Phase of all units', message=FALSE, warning=FALSE, out.width='100%', fig.show = "hold"}
...
```{r chapTappersSol2, fig.align='center', fig.cap='Instantaneous periods. The columns show five different values of ks1m1, the rows show two different values of ks2m2', message=FALSE, warning=FALSE, out.width='100%', fig.show = "hold"}
...
```{r chapTappersSol3, fig.align='center', fig.cap='Relative phase of s1 and s2. The columns show five different values of ks1m1, the rows show two different values of ks2m2', message=FALSE, warning=FALSE, out.width='100%', fig.show = "hold"}
...
```{r chapTappersSol4, fig.align='center', fig.cap='Relative phase of s1 and s2, with phase delay ds1m1 = ds2m2 = 1. The columns show five different values of ks1m1, the rows show two different values of ks2m2', message=FALSE, warning=FALSE, out.width='100%', fig.show = "hold"}
# Set the time points
Solution <- data.frame()
dt = 1/100
time_points <- seq(0, 36.6, by = dt)
...
```{r chapTappersKDdistributions, fig.align='center', fig.cap='Distributions of coupling strength and phase delay parameters for simulations', message=FALSE, warning=FALSE, out.width='100%', fig.show = "hold"}
...
```{r chapTappersKDtable, echo=FALSE}
knitr::kable(cbind(K,D), row.names = TRUE, caption = "K and D parameters for 40 simulations",
             booktabs=T, label = NA ) %>%
...
```{r chapTappers6odePhi12view, echo=FALSE}
States <-  readRDS(file = "Data/chapTappers_States.rds") %>% rename(model = sim)
w <- which(States$time >= .6)
d <- head(States[w-2,])

knitr::kable(d,    booktabs=T, label = NA, 
             caption = paste("States of model 1")) %>%
  kable_styling("striped", font_size = 10)

# \@ref(tab:chapTappers6odePhi12view)
```
...
```{r chapTappersObservationNoise, echo = FALSE}
Observations <- readRDS(file = paste("Data/chapTappers_Observations.rds",sep="")) %>% 
#  select(time, sim, names_indicator) %>%
  rename(model = sim)
d <- head(Observations)
...
```{r chapTappers7generatedODEsample1, echo=FALSE, fig.cap="Dynamic model 1. Generated phase-functions with tapping data", out.width='49%', fig.asp=.75, fig.align='center', fig.show="hold"}

fi <- c("Figures/chapTappers_SimulatedStatesObservations_model_1_RelativePhase.png",
        "Figures/chapTappers_SimulatedStatesObservations_model_1_InstPeriod.png"
)

knitr::include_graphics(fi)

# \@ref(fig:chapTappers7generatedODEsample1)
```
...
```{r chapTappers8generatedODEsample21, echo=FALSE, fig.cap="Dynamic system 21. Generated phase-functions with tapping data", out.width='49%', fig.asp=.75, fig.align='center', fig.show='hold'}
fi <- c("Figures/chapTappers_SimulatedStatesObservations_model_21_RelativePhase.png",
        "Figures/chapTappers_SimulatedStatesObservations_model_21_InstPeriod.png"
)

knitr::include_graphics(fi)

# \@ref(fig:chapTappers8generatedODEsample21)
```
...
```{r chapTappersInData, echo=FALSE}
dd <- Observations %>% dplyr::select(time,model,names_indicator)
knitr::kable(head(dd),    booktabs=T, label = NA, row.names = FALSE,
             caption = paste("Simulated tappers data")) %>%
  kable_styling("striped", font_size = 10)
```
...
```{r chapTappers10datastan1, echo=FALSE, warning=FALSE}
aligned_beat_sec = (600 * 610/10) /1000 
n_gen_times = 100
# ###############################
# 1. Create input dataset to Stan
...
Observations_selected <- Observations[w,] %>% mutate(time = time%%aligned_beat_sec)
# here we select the first dynamic model (sim == 1) 
# and use the columns time, observed, and names_indicator
dat <- Observations_selected %>% filter(sim == 1) %>% 
  dplyr::select(time, observed, names_indicator) %>% arrange(time)
time_gen = seq(0.0001, max(dat$time), length.out = n_gen_times) # time generated
StanData <- list(
  n_times = length(dat$time),
...
```{r chapTappers11datastan3, eval=FALSE, echo = TRUE, cache=FALSE}
...
```{r chapTapperscode1b, eval = FALSE}
functions {
...
```{r chapTappers12stancode2, eval = FALSE}
...
```{stan output.var="stan_model", eval = FALSE, echo = TRUE}
functions {
  vector do_dt(real t, vector y,  real ks1m1, real ks2m2,  real ds1m1, real ds2m2,
  real ds1s2, real ds2s1){
    real pi600 = 2*pi()/.6; // Eigenfrequency s1, m1
...
```{r chapTappersEx1, eval = FALSE, include = TRUE}
    fit <- stan(
      data = StanData,
...
```{r chapTappers12ODEfittedParametersKD, echo=FALSE, fig.cap="Posterior sommary of fitted parameters of the state equation, compared to designed parameters (cicles). Top: coupling strength. Middle: phase delay to metronome. Bottom: phase delay to partner", out.width='100%', fig.asp=.75, fig.align='center', fig.show = "hold"}
  
  knitr::include_graphics("Figures/chapTappers_PosteriorSummary_States_fig.png")
  
  # \@ref(fig:chapTappers12ODEfittedParametersKD)
```
...
```{r chapTappers14KDcompare1, include=FALSE}
  PosteriorSummary_States <-  readRDS(file = "Data/chapTappers_PosteriorSummary_States.rds")
  D <- readRDS(file = "Data/chapTappers_D_5_jun_2024.rds")
  K <- readRDS(file = "Data/chapTappers_K_5_jun_2024.rds")
  KD <- cbind(K,D)
  
  KD_fitted_clear <- PosteriorSummary_States %>% 
  rename(value = .value) %>% 
  dplyr::select(sim, names, value) %>% 
  arrange(sim)
  KD_fitted <- KD_fitted_clear %>% 
  pivot_wider(names_from = names, values_from = value) %>%
  rename(K1=ks1m1, K2 = ks2m2, D1 = ds1m1, D2 = ds2m2, D3 = ds1s2, D4 = ds2s1)  %>%
  dplyr::select(sim,K1,K2,D1,D2,D3,D4)
  
  KD_defined <- KD %>% mutate(ds1s2 = 0, ds2s1 = 0) %>%
  rename(k1 = ks1m1, k2 = ks2m2, d1 = ds1m1, d2 = ds2m2, d3 = ds1s2, d4 = ds2s1)
  
  KD_compare <- cbind(KD_defined,KD_fitted) %>% 
  mutate(Kk1 = K1 - k1,
  Kk2 = K2 - k2,
  Dd1 = D1 - d1,
  Dd2 = D2 - d2) %>%
  dplyr::select(sim, k1,k2,d1,d2, d3, d4,
  K1, K2, D1, D2, D3, D4,
  Kk1, Kk2, Dd1, Dd2) %>% round(2)
  
  #KD_compare
```
  
```{r chapTappers15KDcompare3, echo=FALSE, message=FALSE}
  
  
  bo1 <- which(KD_compare$Kk1 >= .1)
  bo2 <- which(KD_compare$Kk2 >= .1)
  
  knitr::kable(KD_compare, booktabs = T, label = NA, 
  caption = "Defined and fitted parameters of 40 dynamic systems. 
...
$Dd1 = D1 - d1$ and
$Dd2 = D2 - d1$ ") %>% 
  kableExtra::column_spec(c(1,3,7,7,9,13,15,17), border_right = T) %>%
  add_header_above(c(" ", "defined" = 6, "fitted" = 6, "fitted - defined" = 4)) %>%
  kable_styling("striped", font_size = 9) %>%
  row_spec(rbind(bo1,bo2), color = "red")
  
  
  
  
  # \@ref(tab:chapTappers15KDcompare3)
...
```{r chapTappers13ODEfittedParametersKDTable, echo=FALSE}
  SummaryErrors <- readRDS(file = "Data/chapTappers_PosteriorSummary_Errors.rds")
  
  SummaryErrors  %>%
  knitr::kable(caption = "Mean of absolute value of errors, standard deviation of absolute errors, and Q95 range of between designed and estimated compling strength K and phase deley D, without and with designed phase delay", booktabs=T, label = NA ) %>%
  kable_styling("striped", font_size = 10)
  # \@ref(fig:chapTappers13ODEfittedParametersKDTable)
```
...
```{r chapTappers16simFinal, echo=FALSE, fig.cap="Dynamic system 1. Top: Defined discrete observations (circles) versus fitted observatins (gray band). Bottom: Instantaneous period of the defined states and fitted states", out.width='75%', fig.asp=.75, fig.align='center',fig.show = "hold"}
  
  knitr::include_graphics(c("Figures/chapTappers_StateSpaceModel_PosteriorPredictions_Relphase_model_1.png","Figures/chapTappers_StateSpaceModel_PosteriorPredictions_InstPeriod_model_1.png"))
  
#  knitr::include_graphics(c("Figures/chapTappers_StateSpaceModel_PosteriorPredictions_Relphase_model_21.png","Figures/chapTappers_StateSpaceModel_PosteriorPredictions_InstPeriod_model_21.png"))
  # knitr::include_graphics(c("Figures/chapTappers_sim_Stan_Final_13_july_26.png","Figures/chapTappers_sim_Stan_Final_13_july_26_InstPeriod.png"))
  
  
  # \@ref(fig:chapTappers16simFinal)
...
```{r chapTappers16simFinal21, echo=FALSE, fig.cap="Dynamic system 21. Top: Defined discrete observations (circles) versus fitted observatins (gray band). Bottom: Instantaneous period of the defined states and fitted states", out.width='75%', fig.asp=.75, fig.align='center',fig.show = "hold"}
  ...
  # \@ref(fig:chapTappers16simFinal21)
